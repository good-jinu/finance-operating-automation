...
import { getMimeType, readFileAsBase64, readAttachment, readAttachments } from './fileReader'
...
describe('fileReader', () => {
  ...
  describe('getMimeType', () => {
    it('should return the correct MIME type for a given file extension', () => {
      vi.spyOn(path, 'extname').mockImplementation((p: string) => `.${p.split('.').pop() || ''}`)
      ...
      expect(getMimeType('file_without_extension')).toBe('application/octet-stream')
    })
  })
  ...
  describe('readFileAsBase64', () => {
    it('should read a file and return its base64 representation', () => {
      ...
      expect(path.join).toHaveBeenCalledWith(process.cwd(), '.storage/files', filePath)
  ...
  describe('readAttachments', () => {
    ...
    it('should handle errors when reading attachments and skip the failed ones', () => {
      ...
      const base64AnotherGood = Buffer.from(anotherGoodContent).toString('base64')
      ...
      vi.spyOn(fs, 'readFileSync')
        ...
        .mockImplementationOnce(() => { throw new Error('bad file') })
