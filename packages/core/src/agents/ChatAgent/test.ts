...
import { invokeChatAgent, continueChatAgent } from "./ChatAgent";
...
async function testChatAgent() {
	...
	try {
		...
		const response4a = await invokeChatAgent(
			"ABC회사 정보를 찾아주세요.",
		...
		if (response4a.success) {
			console.log("응답:", response4a.content?.toString().substring(0, 100) + "...");
		}
		...
		if (response4b.success) {
			console.log("응답:", response4b.content?.toString().substring(0, 100) + "...");
		}
		console.log("\n" + "=".repeat(50) + "\n");
